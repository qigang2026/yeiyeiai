...
[平台接口] ──────────────────────────
  ...
  └──────────────────────────┘
  [+ 添加 OZON 平台接口]
同一 OZON 店铺可绑定多套 Client ID / API Key，每套接口将拆分为独立店铺行保存。
[启停用] ● 启用 / ○ 停用
                              [取消] [保存]
```

**Wildberries（本土 / 跨境）单接口模式**：

```
[店铺名称] ────────────────────────
[平台+店铺类型] Wildberries/本土店铺 ▼
[本系统币种]   RUB（固定）
[平台接口] ──────────────────────────
  ┌─ Wildberries 平台接口 #1 ┐
  │ [Client ID] ───────────── │
  ...
  └──────────────────────────┘
（不显示 [+ 添加接口] 按钮）
Wildberries 店铺仅支持 1 套 Client ID / API Key 配置。
[启停用] ● 启用 / ○ 停用
                              [取消] [保存]
```
...
- **接口模式**根据平台类型动态切换：
  - OZON（本土 / 跨境）：多接口模式，1 ≤ 接口数 ≤ N，每张卡可独立删除
  - Wildberries（本土 / 跨境）：单接口模式，固定 1 个接口，不显示"添加"按钮，"删除"按钮隐藏
- 切换平台类型时，已填写的接口卡片**不会保留**（重新初始化为 1 张空卡）
- 每套接口需 client_id + api_key，缺一报错
- 多接口模式保存时，**每套接口拆为独立店铺行**（同一店铺名重复出现，行内显示 `N / 总数` + Client ID 脱敏）
- 单接口模式保存时，正常生成 1 行
